...
花了一点时间查了下，发现是由于代码每次直接查了下队首的complete event而没有出队列，这就相当于write+sync的时候，sync其实就没有卡在那里等。于是uring优化write+sync的代码写成write+write的代码，这带宽当然比未优化要好太多。
改了这个bug之后，又发现压测RocksDB老是crash。
